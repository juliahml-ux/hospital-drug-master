...
<head>
  ...
  <title>醫院藥品主檔與臨床處方集智慧管理系統 (115年第二季)</title>
  <!-- Tailwind CSS CDN -->
...
<body class="text-slate-800 antialiased min-h-screen flex flex-col" id="dropZone">
        ...
        <div class="flex items-center space-x-3">
          ...
          <div>
            <div class="flex items-center space-x-2">
              <h1 class="text-lg font-bold text-slate-900 tracking-tight">醫院藥品主檔與臨床處方集智慧管理系統</h1>
            ...
            <p class="text-xs text-slate-500">738 筆院內主檔 • 健保價與給付 PDF • 臨床用法用量 • 禁忌副作用 • 懷孕/肝腎安全分級</p>
          </div>
        </div>
  ...
  <footer class="bg-white border-t border-slate-200 py-4 mt-8">
    <div class="max-w-7xl mx-auto px-4 text-center text-xs text-slate-500 space-y-1">
      <p>醫院藥品主檔與臨床處方集智慧管理系統 • 資料來源：115 年第二季院內藥品清單、嘉義醫院處方集與中央健保署</p>
  ...
  <div id="qrModal" class="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-2xl max-w-sm w-full shadow-2xl border border-slate-100 overflow-hidden transform transition-all text-center">
      ...
      <div class="p-6 flex flex-col items-center space-y-4">
        ...
        <div>
          <p class="text-xs font-semibold text-slate-800">醫院藥品主檔與臨床處方集管理系統</p>
        ...
        tbody.innerHTML = paginated.map((drug, idx) => {{
          ...
          return `
            <tr class="hover:bg-teal-50/40 transition group border-b border-slate-100">
              ...
              <td class="py-3 px-3 bg-teal-50/20 border-x border-teal-100/60 w-52 min-w-[180px]">
    ...
    function openClinicalModal(id) {{
      ...
      if (drug.ruleLink && drug.ruleLink.trim()) {{
        document.getElementById("clinicNhiRuleAction").innerHTML = `
          <a href="${{drug.ruleLink}}" target="_blank" class="px-3 py-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-medium text-xs transition flex items-center space-x-1 shadow-sm">
            <i data-lucide="file-text" class="w-3.5 h-3.5"></i>
            <span>下載健保給付規定 PDF</span>
          </a>
        `;
      }} else {{
        document.getElementById("clinicNhiRuleAction").innerHTML = `
...
注意事項：${{d.note || '無'}}`;
    ...
    function exportToExcel() {{
      ...
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "115年第二季處方主檔(含臨床)");
      const today = new Date().toISOString().slice(0, 10).replace(/-/g, "");
      XLSX.writeFile(wb, `115年第二季院內藥品主檔與臨床處方集_${{today}}.xlsx`);
